...
pipe_collided = [False, False, False]
gap = 150
score = 0
collision_count = 0
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game_over and event.key == pygame.K_r:
                ...
                bird_velocity = 0
                pipe_positions = [600, 800, 1000]
                pipe_heights = [random.randint(250, 375)
                                for _ in pipe_positions]
                frame_count = 0
                bird_frame = 0
                game_over = False
                game_started = False
    ...
    if not game_started:
        ...
        for i in range(len(pipe_positions)):
            ...
            if pipe_positions[i] <= -pipe_width:
                pipe_positions[i] = win_width + random.randint(300, 450)
                pipe_heights[i] = random.randint(250, 375)
                passed_pipes[i] = False
                pipe_collided[i] = False
        ...
        for i in range(len(pipe_positions)):
            window.blit(bottom_pipe_image,
                        (pipe_positions[i], pipe_heights[i]))
        ...
        # Collision detection
        bird_rect = bird_images[bird_frame].get_rect(
            topleft=(260, birdY)).inflate(-10, -10)
        collided = False
        for i in range(len(pipe_positions)):
            bottom_pipe_rect = bottom_pipe_image.get_rect(
                topleft=(pipe_positions[i], pipe_heights[i]))
            ...
            if bird_rect.colliderect(bottom_pipe_rect) or bird_rect.colliderect(top_pipe_rect):
                ...
                if not pipe_collided[i]:
                    collision_count += 1
                    pipe_collided[i] = True
        ...
        collision_text = font.render(
            f"{collision_count}/3 Hits", True, (0, 0, 0))

        window.blit(collision_text, (175, 600))
